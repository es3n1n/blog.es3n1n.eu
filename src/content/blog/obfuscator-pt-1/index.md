...
description: 'introduction to bin2bin pe obfuscation'
date: '2023-12-06'
---

In this series of posts, I will try to cover all the common knowledge you would need to create your [PE](#acronyms) [bin2bin](#acronyms) obfuscator. This year I saw a rise of interest in the topics of software obfuscation and deobfuscation, yet I didn't see any new open-source projects popping up in public, maybe this series of posts could fix that.
...
This is the first part of this series _(and hopefully it won't be the last)_ where I will cover the common info about obfuscators and will write my own obfuscator that could be a good base for a [bin2bin](#acronyms) obfuscator.
...
A reader with no prior knowledge in this field can pose such a question, the answer is simple if we take into account all the [reverse-engineers](#acronyms) that will try to crack your software, would it be a game, an extension for Word or anything else. Generally, obfuscation is applied when you don't want anybody else to understand how your app works internally.
...
Or [bin2bin](#acronyms) for short. As soon as you compile your application with a compiler, on the output you receive a binary, whether it's an [ELF](#acronyms), [PE](#acronyms), or any other file. And then, when you need to obfuscate this app you "feed" the obfuscator this binary with additional debug information ([MAP](#acronyms), [PDB](#acronyms), etc). The obfuscator then does some magic with the binary and on the output, it gives you the obfuscated version of this binary.

A good example of such code protectors could be [VMProtect](https://vmpsoft.com/) and [Themida](https://oreans.com/).

```mermaid
flowchart LR
    A["Input file"] --> B{"bin2bin obfuscator"}
    B --> C["Output file"]
```
...
To implement a [bin2bin](#acronyms) obfuscator you need to first have a deep understanding of how the executable format works ([PE](#acronyms) in our case).

Here is a brief overview of the [PE](#acronyms) file format

### Architecture

There are five architectures that [PE](#acronyms) supports, but we'll cover only two of them, which are the common ones.
...
Essentially, the [PE](#acronyms) file is split into "sections", which store the raw bytes of data and its protection(for example you can't just write into the section with code, because its protection allows you only READ/EXECUTE operations).

Typically there are 3 sections in the [PE](#acronyms) binary that we need to process
...
As you may have heard, the operation system could run the code with [ASLR](#acronyms) to protect the app from some vulnerabilities. Thus, the application that references for example some global vars from the `.data` section should always [dereference](#acronyms) the right address for this var.

Relocations in [PE](#acronyms) are implemented in a way where your code references a pointer to the data with its base set to the default one (which is, stored in the [PE](#acronyms) header).

Then, as soon as your image gets loaded to the memory, the part of OS that loads your [PE](#acronyms) images iterates through all the references to the data that it should relocate and changes the base of this reference.
...
with the default image base set to `0x1400000000`, the [PE](#acronyms) then allocates the image for example at `0x1600000000` and relocates the pointer to

```asm
mov eax, dword ptr [0x1600000017]
```

To do that, the [PE](#acronyms) stores a relocation table inside of it with all the references, that the OS should process.

However, in x64 applications, the more common way compilers are placing references, instead of such relocations is [RIP](#acronyms) addressing. It means that instead of placing a direct address to this symbol, it would rather calculate a difference between the current instruction address and the symbol that it needs to reference, and will use this offset. So that you don't even need to relocate anything!
...
- [linux-pe](https://github.com/can1357/linux-pe): A [PE](#acronyms) file format parser.
...
```mermaid
flowchart TB
    CLI[CLI Arguments] --> CP{Config&nbsp;Parser}
    CP --> FP{Func&nbsp;Parser}
    CP --> A{Analysis}
    FP --> A
    A --> S{Scheduler}
...
- Architecture check
- [PE](#acronyms) magic constants checks
...
Or `func_parser`, is the module that parses function start/end [RVAs](#acronyms) from various sources, for now only [MAP](#acronyms) and [PDB](#acronyms) files are supported. The only thing that my function parsers lack is the [name demangler](#acronyms).
...
flowchart TB
    ...
    FP --> MP{MAP&nbsp;parser}
    FP --> PP{PDB&nbsp;parser}
    MP --> FL["Functions list&nbsp;(range,&nbsp;name,&nbsp;...)"]
    PP --> FL
```

### PDB

[PDB](#acronyms) is the program database format that contains a lot of stuff, somehow this file format is a bit obscure, even though it is pretty common. While I was developing my implementation of the [PDB](#acronyms) parser I referenced the [LLVM Documentation](https://llvm.org/docs/PDB/index.html) and [raw_pdb](https://github.com/MolecularMatters/raw_pdb) project.

There are two versions of [PDB](#acronyms) files, version 2 and version 7. The most common one in 2023 is version 7 and this is the only version that my [PDB](#acronyms) parser supports.

### MAP

There are two versions of [MAP](#acronyms) files, [MSVC](#acronyms) version and [GCC](#acronyms) version. My parser supports only the [MSVC](#acronyms) version. However, implementing a parser for [GCC](#acronyms) version is fairly straightforward. If I have time before publishing this article, I will add it.

## Disassembling

The library I am using([zasm](https://github.com/zyantific/zasm)) supports the [disassembling](#acronyms) of the x86 code to a single container(`zasm::Program`), and then [assembling](#acronyms) instructions from this container back. And to do that I implemented my wrapper for a wrapper. Its name is `EbinAsm`, `easm` in short form.
...
- Program [assembler](#acronyms)
- One-line JMP encoder
- Node cursor (simplify the usages of assembler->setCursor)
- Program instructions info debug dump
- [Disasm](#acronyms) block of code
- [Disasm](#acronyms) single instruction
- [GP](#acronyms) registers converter:
  - 32-bit to 64-bit
  - 64-bit to 32-bit
  - 64-bit to 16-bit
  - 64-bit to 8-bit
- [JCC](#acronyms)/JMP matcher
- [RET](#acronyms) matcher
- Instruction that affects [IP](#acronyms) matcher
- [JCC](#acronyms)/JMP follower
- [Operand](#acronyms) size calculator
- `zasm::Reg` converter:
  - to `zasm::x86::Gp`
  - to [Root GP](#acronyms) `zasm::x86::Gp`
- [SP](#acronyms) matcher
...
Store options for the [MAP](#acronyms)/[PDB](#acronyms) parsers.
...
Code analysis consists of six phases within the [Basic Block](#acronyms) decomposition and four analysis passes.
...
```mermaid
flowchart TB
subgraph "Decomp pipeline"
    direction TB
    DE[Decomp] --> JTP[JumpTables&nbsp;Parsing] --> SPL[Split] --> JTEXP[JumpTables&nbsp;Expansion]
    JTEXP --> JMPINS[JMP&nbsp;Insertion] --> TREEUPD[Tree&nbsp;Update]
end

subgraph "Pass pipeline"
    direction LR
    REF[Ref&nbsp;Collector] --> LOOKUP[Lookup&nbsp;Collector] --> LRU[LRU&nbsp;Registers] --> LABEL[Label&nbsp;Refs]
end

TREEUPD --> REF
LABEL --> OUT[(Analysed&nbsp;Function)]
```

### Basic Block decomposition

[Basic blocks or BBs](#acronyms) are the sequences of instructions in code that have a single entry point and at most two exit points.

![bbs](./pics/bbs.jpg)

To decompose a basic block we need to [disassemble](#acronyms) instructions one by one until we hit a [JCC](#acronyms)/JMP.

At this moment BB decomposition also includes the logic for [Jump Tables](#acronyms) expansions.

#### Phase 1: Decomposition

The whole [BB](#acronyms) decomposition is made in a single recursive function that will assemble a new BB until its end is hit, and then it will start decomposing a new one.
...
So what we need to do, is find the reference to a [JumpTable](#acronyms), get its size, remove the JMP instruction and chain JZs instead.
Sounds trivial, however, it isn't as easy as it sounds because as always the hardest things are these small details. Probably this is why [Themida](https://oreans.com/) still doesn't support the code with [JumpTables](#acronyms) in it.

##### Phase 2-1: JumpTables collector

To expand something, we first need to find something, right? To collect all the [JumpTables](#acronyms) we are first iterating through all the [BBs](#acronyms) and looking for instructions where we were unable to estimate the [JCC](#acronyms)/[JMP](#acronyms) destination.

We are unable to estimate it because our decomposer doesn't symbolically execute the code that it's analyzing, thus it doesn't know what's stored in the [JMP](#acronyms) operand because it's a register.
...
- The so-called `load_index` instruction that looks like this, in my example, is `mov ecx, dword [rdx+rax*4+0x1188]`, where the `0x1188` is the [JumpTable](#acronyms) start [RVA](#acronyms).
...
This part is a bit tricky because normally you'll see [JE](#acronyms)/[JNE](#acronyms) instructions that would check for the [JumpTable](#acronyms) bounds first. Like this `cmp eax, 0x6; ja somewhere` in my example.
...
But with just a straight brute force there could potentially be some collisions with the other [JumpTables](#acronyms), that we can hit.

So what we have to do is first find all the start addresses of the [JumpTables](#acronyms), then brute-force its entries while checking whether we're hitting an entry from a different [JumpTable](#acronyms) or not.
...
If you understood how the [BB](#acronyms) decomposition works, then you may have understood its general flaw. The way it disassembles and decomposes basic blocks linearly would produce multiple duplications of the basic blocks.
...
The output basic blocks from the [BB](#acronyms) decomposer would be like this:
...
As you can see, there's a duplicated [BB](#acronyms) that contains the same instruction in multiple [BBs](#acronyms). This instruction is RET and that's because we're following all the branches and defining new [BBs](#acronyms) if we don't see any [BB](#acronyms) that starts at this address.

To fix that, we have to find all the intersections between all the [BBs](#acronyms) and split them so that we would have basic blocks like this, for the example below:
...
Now, when we are sure that we've already collected all the needed information about [JumpTables](#acronyms) within the function, we can start expanding them to the chained [JZ](#acronyms) expressions.

The whole process of this is quite easy, all we have to do is:

- Transform the load_index instruction from `mov ecx, dword [rdx+rax*4+0x1188]` to `lea ecx, dword [rax*4]` so that we would get instead of the case RVA, its offset in the [JumpTable](#acronyms)
- Iterate through all [JumpTable](#acronyms) entries, then:
  - Compare the index
  - Analyze the case
  - Jump to the newly analyzed [BBs](#acronyms) from the new case

And in the end, the whole "unwrapped" [JumpTable](#acronyms) would look as follows:
...
In the analysis module, it collects all the "non-default" [CF](#acronyms) changes of the instruction and caches it, for example, for the instruction `JMP` there would be 1 [CF](#acronyms) changer entry with its type set to `JMP`, for `JE` instruction there would be two [CF](#acronyms) changers, one if the condition is met and one if condition isn't met.

Thus, at the time of [JumpTable](#acronyms) expansion, there could be some cases where it was unable to detect this info, and to do this, the analyzer is re-scheduling the update of this info, and we have to update it manually.

##### Phase 4-3: Splitting BBs

Since after the [JumpTable](#acronyms) expansions, we added some new basic blocks, we also need to split them in the same way as we did it before

#### Phase 5: Jmp insertion

After all these [JumpTable](#acronyms) expansion phases, some fresh new [BBs](#acronyms) aren't ending with a jump and their next [BBs](#acronyms) wouldn't be the ones, that it expects.

To fix this, we have to iterate through all [BBs](#acronyms), check its next nodes and verify that it is the one that it expects, and insert a jump to the right one, if not.

#### Phase 6: Tree update

In the [BB](#acronyms) there are two vectors with [BB](#acronyms) [successors](#acronyms) and [predecessors](#acronyms). After all the analysis phases there could be some invalid data about the [successors](#acronyms), and [predecessors](#acronyms) after we split them.

To make sure that this info is up to date, we need to iterate through all the [CF](#acronyms) changes and keep track of [successors](#acronyms), then after we have the list of [successors](#acronyms), we can update the list of [predecessors](#acronyms).

### Observer

In the [zasm](https://github.com/zyantific/zasm) library, we can create our custom `Observers` that would be notified as soon as a new node gets inserted/deleted to the program, by using these callbacks we can append new instructions to the [basic blocks](#acronyms).
...
Store the instructions in an `unordered_map` with its [RVA](#acronyms) set as the key.
...
Nice. Now we need to add the `NOP` instruction, to do that we have to get the assembler whose cursor is set to the beginning of the [BB](#acronyms) and call the `nop` function from the assembler object.
...
void startup_scheduler() {
    ...
    scheduler.register_transform<transforms::Nops>(); // [!code ++]
}
```
...
For example, I want to transform both Function and [Basic Blocks](#acronyms) from a single transform.
...
- Function
  ...
  - [BB](#acronyms) internal provider
  - [zasm](https://github.com/zyantific/zasm) machine mode
  - Var allocator
- [Basic Block](#acronyms)
  - [zasm](https://github.com/zyantific/zasm) machine mode
  - (optional) start/end [RVAs](#acronyms)
  - List of instructions
  - List of attached labels (for addressing)
  - List of successors
  - List of predecessors
- Instruction
  - (optional) [RVA](#acronyms)
  ...
  - Attached [basic Block](#acronyms) reference
  - Control flow changes info
    - Type:
      - `JCC_CONDITION_MET`
      - `JCC_CONDITION_NOT_MET`
      - `JMP`
    - Destination [BB](#acronyms)
  - Relocations info
    - Type:
      - `HEADER`
      - `IP`
    - Immediate value [RVA](#acronyms)
...
You may think that the iterations through nodes/instructions/[bbs](#acronyms) could be optimized and done only once, but you would be wrong, as that would break the appliance order
...
An obfuscation that inserts an [opaque predicate](#acronyms) to the [Basic Block](#acronyms) thus creates a second "dummy" successor branch of the [Basic Block](#acronyms), that will never be executed.
...
To be precise, I used [z3](https://github.com/Z3prover/z3) to generate [tautologies](#acronyms). This is not ideal and all of these generated [tautologies](#acronyms) in the end are mostly discarding bits, but it was a nice thing to implement.
...
There is also a `mode` variable that I added, which could also instead of generating [opaque predicates](#acronyms) generate a math expression using `mathop`, check for a random value and make two identical [BBs](#acronyms). This will result in semantically the same code execution, although the output in decompilers would look quite messy because it would treat them as different paths of code, although they're doing the same thing.

![bcf](./pics/bcf.png)

#### Substitution

An obfuscation that transforms a math expression, to its equivalent that could look a bit more confusing for the [reverse engineer](#acronyms).
...
![substitution](./pics/substitution.png)
...
![ida_xchg](./pics/ida_xchg.png)

##### Breaking BInja symbolic execution

Breaking optimizations in [binja](https://binary.ninja/) was pretty tough(without using any [CF](#acronyms) tricks) so definitely a big shout out to Vector35.
...
![microcode_exc](./pics/microcode_exc.png)

But of course, we can't just append this instruction to any [Basic Block](#acronyms) as this would produce a crash on a bare metal too, not only when we decompile this function.

To "fix" that, I applied the same technique that I used for opaque predicates. It generates the same successor, inserts an opaque predicate, and inserts this instruction somewhere in the new [BB](#acronyms) that would never be executed because of the opaque predicate.
					   ...
					   const RangeList &uselim)
{
  ...
  if (lastaddress.getOffset() < addr.getOffset()) { // [!code ++]
    string msg = "Symbol "; // [!code ++]
    msg += sym->getName(); // [!code ++]
    msg += " extends beyond the end of the address space"; // [!code ++]
    throw LowlevelError(msg); // [!code ++]
  } // [!code ++]
...
Achieving this is easy enough, all we have to do is [dereference](#acronyms) something at `-1`.
...
![ghidra_oob](./pics/ghidra_oob.png)
...
Before linking, the obfuscator estimates the whole code size of all obfuscated functions and creates a new `READ/EXECUTE` [PE](#acronyms) section.
...
[PE](#acronyms) rebuilding was a bit harder than linking because it required me to write all the code by myself.

After all the previous stages we already have all the parsed [PE](#acronyms) information and all the new sections, all we need to do is create an output [PE](#acronyms) file.

#### Relocations update

Before updating relocations, it might be a good thing to erase previous ones and erase the whole section that contains it(if needed). To do that, we have to iterate through all the sections of the [PE](#acronyms) and find the one that contains relocations. Afterward, we need to fill it with zeroes and check whether we filled all sections with zeros.

Now, we need to assemble the new relocations to our new section with relocations. This is quite trivial because all we need to do is sort them by their base [RVAs](#acronyms) and then just serialize them as `uint16_t`.

#### Header initialization

Before copying all the sections, we have to initialize the [PE](#acronyms) header, update its size, and reserve a new buffer size.
...
To keep track of the consistency of the [PE](#acronyms) image, there's a `Checksum` field in the [PE](#acronyms) header. Which is calculated by adding all the `uint16_t` chunks of the [PE](#acronyms) header. You can refer to the [linux-pe](https://github.com/can1357/linux-pe) sources to check the implementation of this algorithm.

All processes are complete! Now, write the output binary to the disk and exit.

# Why this project is better

In the preface, I said that some existing public [bin2bin](#acronyms) obfuscators are not well-designed and have limited scalability. This implies that my project, in contrast, has better scalability, and this is indeed the case.

If we compare the current state of the obfuscator to other publicly available open-source similar projects, we can see that most of these projects are missing a lot of the crucial analysis parts, even [BB](#acronyms) decomposition which is needed for a lot of [CF](#acronyms) transforms. As a result, adding new features to these projects wouldn't be as straightforward.
